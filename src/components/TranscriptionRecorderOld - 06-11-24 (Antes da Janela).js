...
const TranscriptionRecorder = () => {
  ...
  const startRecording = async (evt) => {
    ...
    const now = new Date();
    const sessionDate = format(now, "dd/MM/yyyy - HH:mm:ss", { locale: ptBR });

    // Criar um novo documento de sessão no Firebase
    try {
      ...
      console.log('Sessão criada no Firebase com ID:', sessionDocRef.id);
    } catch (error) {
      console.error('Erro ao criar a sessão no Firebase:', error);
    }
      ...
      micSocket.onmessage = async (event) => {
        ...
        if (data?.event === 'transcript' && data.transcription) {
          if (data.type === 'final') {
            ...
            // Salvar a transcrição completa no Firebase
  ...
  const saveTranscription = async (data) => {
    if (!sessionId) {
      console.error('ID da sessão não está definido.');
      return;
    }

    try {
      await addDoc(collection(db, 'sessions', sessionId, 'transcriptions'), {
        event: data.event,
        ...
        duration: data.time_end - data.time_begin,
        createdAt: serverTimestamp(),
      });
      console.log('Transcrição salva no Firebase:', data.transcription);
    } catch (error) {
